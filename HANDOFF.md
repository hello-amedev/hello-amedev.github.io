...
### 第4コミット(CV エリア修正)
16. **価格エリアのコピー刷新**: リード文を「試用中の機能制限はありません。チームで一通り
    使ったうえで、買い切り(目安 2,000 円台・1 ユーザー 1 ライセンス)で導入できます。
    月額課金はありません。」→「サブスク不要、買い切りでずっと使えます。30 日間すべての
    機能を無料でお使いいただけます。」に短縮(3.8 節に反映済み)。
17. **CTA ボタンのベージュ残留を修正**: `--paper-night`(旧 `#F1EEE6`)が紙色クールグレー化
    (第 1 コミット)の際に未追随で、価格エリアの暗パネル上で唯一ベージュのまま残っていた
    (ボタン背景・見出し・リード文字色などに使用)。`--paper-deep` と同系の `#EEF1F5` に統一し、
    関連する rgba 直書き 4 箇所(`--line-night` 含む)も合わせて置換。
18. **価格 CTA 下のメタ表記をヒーローと統一**: 「月額課金なし・Windows 専用」→
    「v0.13.1(2026-07-18)・Windows 10 以降対応」(ヒーロー下のノートと同一文言に統一)。

### 第5コミット(CV エリア追加修正・あめさんの指摘反映)
19. **ページ内メニューの「価格」→「ダウンロード」に変更**: 価格エリアから金額表示がなくなり
    「価格」という表記が実態と合わなくなったため。リンク先(`#price`)は変えず、ヘッダー・
    モバイルオーバーレイ・ハブ側共通ヘッダー(`SiteHeader.astro`)の計 3 箇所を修正
    (公開時点では DL リンクは未接続だが、意味は通るとの判断で採用)。
...
### 3.8 価格(2026-07-20 第 4・5 コミットでコピー改訂)
- **タイトル**: 30 日間、無料で試せます。
...
- **紙色パレット変更時は「暗パネル用の対の変数」も忘れず追随させる**: `--paper`/`--paper-deep`
  ...
  (2026-07-20 第 4 コミットで発覚・修正)。`--night`(墨のダーク背景)は据え置きでよいが、
  その上に乗る「紙色」側の変数は必ずセットで見直す
