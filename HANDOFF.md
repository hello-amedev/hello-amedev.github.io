...
## 0. 次の Claude へ(2026-09-05 Drive DAM 掲載コピー書き直しセッション終了時)

### Drive DAM の掲載コピーを書き直した(2026-09-05、`608cb61` / `3a8a750` / `262a7ed`)

起点はあめさんの気づき —— **「取り込まない」とだけ言っても、何と比べて良いのかが
伝わらない**。オンラインストレージ型の DAM と、ローカル動作の DAM、その両方との
違いを言わないと特徴として成立していなかった。

1. **「Drive DAM とは」に既存 DAM 2 種との対比を追加**。専用サーバーに画像を
   取り込む型と、手元の PC にタグを保存する型を挙げ、**Drive DAM はどちらでもない**
   と受ける形。**欠点は並べず、次の文で打ち返す**構成にしてある(「取り込みは不要」が
   前者を、「フォルダを共有すればタグも共有される」が後者を打ち返す)。他社を貶さずに
   違いが伝わる。**ここを短くしすぎると差別化が伝わらなくなる**ので、削るときは注意
2. **「フォルダ構成も画像ファイルも変更しません」は事実に反していたので直した**。
   `_dam` と `_dam_thumbs` が各フォルダに増える。**同じページの FAQ とは元々
   食い違っていた**(FAQ 側は「フォルダ数が多い場合は同名フォルダを検索してすべて
   削除してください」と正直に書いている)。方針は **フォルダが増えることを隠さず、
   それ自体が差別化の理由だと説明する** —— drive-dam 側のストア掲載文と同じ論法に
   揃えてある(あちらは `docs/store-listing.md`)
3. **個人利用も明記**(「個人でもチームでも」)。タグを画像と同じフォルダに置くという
   1 つの仕組みから、チームでは共有、個人では持ち運びという 2 つの利点が出る
4. **特徴 2 は一度「タグを持ち運べる」を主軸にして 5 文に膨らみ、差し戻した**
   (あめさん指摘: 持ち運べることはメインの利点ではない)。**取り込まないことの利点は
   「いまのフォルダのまま、すぐ使える」に絞る**。共有の話は特徴 3 の担当なので
   ここで繰り返さない。見出しも「ローカルでも、オンラインストレージでも」
   (対応範囲の説明で差別化になっていない)から
   「取り込まないから、いまのフォルダのまま。」へ
5. **v1.3.5 が `releases.ts` に載っていなかった**(Store では公開済み)。
   v1.3.6 と合わせて追加した。**バージョンを公開したら `releases.ts` に足す**手順を
   落とさないこと。ヒーロー・価格メタ・JSON-LD の版表記はここから自動追随する

**日英中の 3 言語とも反映済み。`npm run build` 通過、本番反映も確認済み**
(`https://ame-dev.com/drivedam/`)。

## 0-0. 次の Claude へ(2026-09-04 独自ドメイン移行セッション終了時)
